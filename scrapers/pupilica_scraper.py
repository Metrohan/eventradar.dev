...
def scrape_pupilica_events() -> List[Dict[str, Any]]:
    ...
    for rec in records:
        ...
        instructors = ", ".join(
            [
                i.get("name", "")
                for i in (rec.get("instructor_ids") or [])
                if i.get("name")
            ]
        )
        ...
        events.append(
            {
                ...
                "url": str(
                    rec.get("website_absolute_url") or "https://pupilica.com/events"
                ),
                "image_url": (
                    f"https://learn.pupilica.com/web/image/event.event/{rec.get('id')}/cover"
                    if rec.get("id")
                    else DEFAULT_IMAGE
                ),
                "source": "Pupilica",
