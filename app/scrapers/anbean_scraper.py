...
def scrape_anbean_events():
    ...
    if not event_cards:
        print(
            "Anbean Kampüs sayfasında etkinlik kartları bulunamadı. Selector'ı kontrol edin."
        )

    for card in event_cards:
        main_link_element = card.find("a", title=True)
        link = (
            main_link_element["href"].strip()
        ...
        title = (
            main_link_element["title"].strip()
            if main_link_element and "title" in main_link_element.attrs
        ...
        if date_items:
            for item in date_items.find_all("div", class_="mini-eventCard-dateItem"):
                ...
                if len(spans) == 2 and (
                    spans[0].text.strip() == "Son Başvuru"
                    or spans[0].text.strip() == "Son Kayıt"
                ):
        ...
        if need_detail_lookup and link:
            try:
                ...
                if not detail_date and detail_soup.title and detail_soup.title.string:
                    ...
                    m = re.search(
                        r"(\d{1,2}[\.\s]\w+\s\d{4}(?:\s\d{1,2}[:\.]\d{2})?)|(\d{1,2}[\./]\d{1,2}[\./]\d{4}(?:\s\d{1,2}[:\.]\d{2})?)",
                        title_text,
                    )
                    if m:
                ...
                if not detail_date:
                    ...
                    label_match = re.search(
                        r"Son\s*(Kayıt|Başvuru)[^\d]{0,60}([\d\s:\.]+\w+\s+\d{4}(?:\s+[\d:\.]{4,5})?)",
                        detail_text,
                        flags=re.IGNORECASE,
                    )
                    if label_match:
                ...
                if not detail_date and search_dates:
                    dates_found = (
                        search_dates(
                            " ".join(detail_soup.stripped_strings), languages=["tr"]
                        )
                        or []
                    )
                    if dates_found:
        ...
        status_closed_badge = card.find(
            "span", class_="mini-eventCard-statusBadge text-danger"
        )
