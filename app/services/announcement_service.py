...
class AnnouncementService:
    ...
    def get_announcements(self) -> List[Announcement]:
        ...
        return (
            self.db.query(Announcement).order_by(Announcement.created_at.desc()).all()
        )

    def get_latest_announcement(self) -> Optional[Announcement]:
        """Get the latest announcement"""
        return (
            self.db.query(Announcement).order_by(Announcement.created_at.desc()).first()
        )
    ...
    def create_announcement(
        self, announcement_data: AnnouncementCreate
    ) -> Announcement:
        """Create a new announcement"""
        db_announcement = Announcement(
            title=announcement_data.title, message=announcement_data.message
        )
